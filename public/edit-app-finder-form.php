...
if (isset($_POST['btnUpdate'])) {
    ...
    $sql = "UPDATE app_find SET title='$title',link='$link' WHERE id = '$ID'";
    $db->sql($sql);
    $categories_result = $db->getResult();
    if (!empty($categories_result)) {
        $error['add_menu'] = " <span class='label label-danger'>Failed</span>";
    } else {
        $error['add_menu'] = " <span class='label label-success'>App Finder Updated Successfully</span>";
    }
    if (isset($_FILES['image']['size']) && $_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
        ...
        $sql = "UPDATE app_find SET `image`='$upload_image' WHERE `id`='$ID'";
        $db->sql($sql);
        ...
        if ($update_result == 1) {
            $error['update_menu'] = " <section class='content-header'><span class='label label-success'>App Finder updated Successfully</span></section>";
        } else {
...
$sql_query = "SELECT * FROM `app_find` WHERE id = '$ID'";
$db->sql($sql_query);
$res = $db->getResult();
foreach ($res as $row)
$data = $row;
...
<h1>Edit App Finder <small><a href='app-finder.php'> <i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to App Finder</a></small></h1>
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="box box-primary">
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form id='edit_app_find_form' method="post" enctype="multipart/form-data">
                    <div class="box-body">
                        <div class="row">
