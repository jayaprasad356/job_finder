...
if (isset($_POST['btnAdd'])) {
    ...
    if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
        ...
        $sql = "INSERT INTO app_find (title,link, image) VALUES ('$title','$link', '$upload_image')";
        $db->sql($sql);
    } else {
        // Image is not uploaded or empty, insert only the title
        $sql = "INSERT INTO app_find (title,link) VALUES ('$title','$link')";
        $db->sql($sql);
    }
    $result = $db->getResult();
    if (!empty($result)) {
    ...
    if ($result == 1) {
        $error['add_video'] = "<section class='content-header'>
                                            <span class='label label-success'>App Finder Added Successfully</span> </section>";
    } else {
...
<section class="content-header">
    <h1>Add App Finder <small><a href='app-finder.php'> <i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to App Finder</a></small></h1>
